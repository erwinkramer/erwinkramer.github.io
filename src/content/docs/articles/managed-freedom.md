...
[UpCloud](https://upcloud.com/) belongs in a smaller, more restrained category. It has managed Kubernetes with a modern cadence, S3-compatible object storage, managed PostgreSQL, Valkey, and OpenSearch, plus log export to OpenSearch or Rsyslog with TLS and OpenTelemetry guidance built around an owned collector pipeline. The weaker point is identity: unlike Exoscale, OVHcloud, or Cyso, UpCloud does not make external OIDC for managed Kubernetes a visible part of the platform story. That smaller surface can be a strength for teams that want more control, but access, scaling, networking, and database tuning need more deliberate setup.
