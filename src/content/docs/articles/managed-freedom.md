...
**Exoscale** has the core primitives: managed Kubernetes, S3-compatible object storage, managed PostgreSQL, networking, DNS, and load balancing. Its PostgreSQL API lists PostgreSQL 17 and 18 as supported major versions for service creation and upgrade checks. Its Kubernetes service supports external OIDC providers, including setups based on Dex or Keycloak, so identity does not have to become an Exoscale account boundary. Its database service supports Prometheus, Rsyslog, and OpenSearch integrations; those are not native OTLP outputs, but they are neutral enough to bridge into an OpenTelemetry Collector-owned pipeline without making Exoscale's console the operational center. Exoscale's managed Kubernetes Pro offering can also send control-plane audit records to a webhook endpoint. That is the healthy version of the concern described in [Decoupled by Design](/articles/decoupled-by-design/): dependencies are fine when the boundaries stay visible.
...
**Aruba Cloud** belongs in this broad-enterprise reading too, with an Italian center of gravity. It is relevant when Italian data residency, public-sector qualification, VMware migration, or hybrid infrastructure matter, but the two hard caveats are database freshness and signal ownership: its docs list PostgreSQL 13.4, a 2021 release, while logging and monitoring read as provider-console-first, with charts, exports, and activity-log downloads rather than documented Prometheus, OpenTelemetry, or log-forwarding boundaries.

**IONOS Cloud** and **T Cloud Public** belong closer to this broad-enterprise reading as well. They manage real primitives, but the shape is heavier: more catalog, more governance surface, and more chance that the cloud account becomes the natural center of decisions.
