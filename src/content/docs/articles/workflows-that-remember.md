...
For a .NET-centered organization, Durable Task Extensions is the most sympathetic starting point: code-first, self-hostable, database-backed, dashboard included, MIT-licensed, and identity-aware through OIDC and ASP.NET Core authorization. Cadence is the serious open-source platform challenger when Go or Java workers are acceptable and the organization is willing to operate a broader runtime. For broader polyglot or adjacent open-source requirements, Elsa, Dapr Workflow, Wexflow, Infinitic, pg_durable, and TanStack Workflow may still be worth studying, but only with explicit maturity, fit, and operator-control tradeoffs. They are not the clean default for a free, complete, operable sovereign workflow engine.
